...
def Extension_RoliePoly(params, t, epd, manOut=False, rtol=1e-13, atol=1e-13):
    # all with SI units
    ...
    def dydt_Extension_RoliePoly(t, y, epd, L, tau_d, tau_s, b, dl):
        if (y[2] > L) and (L > 0):
            return [0, 0, 0]
        else:
            # [R, Srr, Szz, lmd]
            ...
            dydt = np.array([ -epd(t)*Srr - 2*epd(t)*(Szz-Srr)*Srr - relaxTerm*(Srr-1/3),
                             2*epd(t)*Szz - 2*epd(t)*(Szz-Srr)*Szz - relaxTerm*(Szz-1/3),
                                              epd(t)*(Szz-Srr)*lmd - f/tau_s*(lmd-1) - relaxTerm*((lmd**2)-1)/(2*lmd)])
            return dydt
    ...
    if type(t) == np.ndarray:
        sol = solve_ivp(dydt_Extension_RoliePoly, (t.min(), t.max()), [Srr0, Szz0, lmd0], t_eval=t, args=(epd, L, tau_d, tau_s, b, dl),
                        rtol=rtol, atol=atol)
    else:
        sol = solve_ivp(dydt_Extension_RoliePoly, (0, 1.2*t), [Srr0, Szz0, lmd0], args=(epd, L, tau_d, tau_s, b, dl),
                        rtol=rtol, atol=atol)
    Srr = sol.y[0, :]
    Szz = sol.y[1, :]
    lmd = sol.y[2, :]
